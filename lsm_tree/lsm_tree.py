...
class Layer:
    ...
    def add(self, key: str, value: str):
        if self.is_full():
            logger.debug(f"Layer is full, aborting add operation...")
            return
    ...
    def is_full(self): 
        logger.debug(f"Layer has size {self.size} out of {self.max_size}.")
        return self.size == self.max_size

    def search(self, key: str):
        if not self.bloom_filter.might_contain(key):
            logger.debug(f"Bloom filter: key '{key}' definitely not in this layer")
            return None
        
        logger.debug(f"Bloom filter: key '{key}' might be in this layer, checking...")
        ...
        logger.debug(f"False positive from Bloom filter")
        return None
...
class DiskLayer(Layer):
    ...
    def save_to_disk(self):
        ...
        with open(self.filepath, 'wb') as f:
            pickle.dump(data, f)
        logger.info(f"Saved layer {self.layer_id} to disk: {self.filepath}")
    ...
    def load_from_disk(cls, layer_id: int, storage_dir: str = STORAGE_DIR) -> Optional['DiskLayer']:
        ...
        logger.info(f"Loaded layer {layer_id} from disk: {filepath}")
        return layer
...
class LSMTree:
    ...
    def _load_existing_layers(self):
        ...
        if len(self.layers) > 1:
            logger.info(f"Loaded {len(self.layers) - 1} disk layers from storage")

    def add(self, key: str, value: str):
        self.layers[0].add(key, value)

        if self.layers[0].is_full():
            logger.debug("Memory buffer is full. Merging...")
...
def merge(layers: list[Layer | MemoryBuffer | DiskLayer], layer_index: int, storage_dir: str = STORAGE_DIR):
    ...
    if layer_index == len(layers) - 1:
        logger.debug(f"Adding new layer with max_size={R * layers[-1].max_size}")
        next_layer_id = layer_index + 1
        next_layer = DiskLayer(R * layers[-1].max_size, next_layer_id, storage_dir)
    else:
        logger.debug(f"Merging layer {layer_index} with next layer {layer_index + 1}")
    ...
    for key, value in layer.objects.items():
        if isinstance(layer, MemoryBuffer):
            if not value[1]:
                logger.debug(f"Merging key={key} and value={value[0]} to next layer.")
                if key not in merged_layer.objects:
                    merged_layer.size += 1
                merged_layer.objects[key] = value[0]
                merged_layer.bloom_filter.add(key)
        else:
            logger.debug(f"Merging key={key} and value={value} to next layer.")
            if key not in merged_layer.objects:
                merged_layer.size += 1
            merged_layer.objects[key] = value
            merged_layer.bloom_filter.add(key)
    ...
    if merged_layer.is_full():
        logger.debug("Next layer is full, merging next layer...")
        merge(layers, layer_index + 1, storage_dir)


def search_layers(layers: Sequence[Layer | MemoryBuffer | DiskLayer], key: str):
    logger.debug(f"Searching for key='{key}' across {len(layers)} layers...")
    
    for i, layer in enumerate(layers):
        logger.debug(f"Searching in layer {i}...")
        value = layer.search(key)
        if value is not None:
            logger.debug(f"Found key='{key}' with value='{value}' in layer {i}")
            return value
    
    logger.debug(f"Key='{key}' not found in any layer")
    return None


def range_search_layers(layers: Sequence[Layer | MemoryBuffer | DiskLayer], start_key: str, end_key: str):
    logger.debug(f"Range search for keys from '{start_key}' to '{end_key}' across {len(layers)} layers...")
    ...
    for i, layer in enumerate(layers):
        ...
        logger.debug(f"Found {len(layer_results)} keys in layer {i}")
    
    logger.debug(f"Total unique keys found: {len(result)}")
    return result


def run_program():
    logging.basicConfig(filename='lsm.log', level=logging.DEBUG)
    logger.info("LSM leveled compaction started.")
    logger.info(f"Using R={R} as size modifier")

    lsm = LSMTree()
    logger.info("Created memory buffer. (layer 0)")
    ...
    while True:
        ...
        if command == "add":
            ...
            logger.info(f"Adding key={key} and value={value} to memory buffer.")
            lsm.add(key, value)
        elif command == "remove":
            key = input("Enter key: ")
            logger.info(f"Removing key={key} from memory buffer.")
            lsm.remove(key)
        elif command == "get":
            key = input("Enter key: ")
            result = lsm.get(key)
            ...
            if results:
                logger.info(f"Found {len(results)} keys:")
